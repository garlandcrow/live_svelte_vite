defmodule LiveSvelte.Reload do
  ...
  def vite_assets(assigns) do
    ...
    ~H"""
    <%= if Application.get_env(:live_svelte, :vite_host) do %>
      <script type="module" src={LiveSvelte.SSR.ViteJS.vite_path("@vite/client")}>
      </script>
      <link :for={path <- @stylesheets} rel="stylesheet" href={LiveSvelte.SSR.ViteJS.vite_path(path)} />
      <script :for={path <- @javascripts} type="module" src={LiveSvelte.SSR.ViteJS.vite_path(path)}>
      </script>
    <% else %>
